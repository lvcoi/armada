...
import { FLEET, CELLS } from './constants.js';
 ...
 */
export function validateFleet(ships, terrainId) {
  ...
  for (const spec of ships) {
    ...
    if (!isCell(spec.anchor)) return { ok: false, error: `bad anchor for ${def.type}` };

    const cells = shipCells(spec.anchor, spec.dir, def.len);
    if (!cells) return { ok: false, error: `${def.name} runs off the grid` };

    for (const c of cells) {
      if (isLand(terrainId, c)) return { ok: false, error: `${def.name} overlaps land` };
...
export function canPlace(anchor, dir, len, terrainId, occupied) {
  const cells = shipCells(anchor, dir, len);
    if (!cells) return null;
  for (const c of cells) {
    if (isLand(terrainId, c)) return null;
...
export function legalDirections(anchor, len, terrainId, occupied) {
  return ['h', 'v'].filter((dir) => canPlace(anchor, dir, len, terrainId, occupied) !== null);
}
 ...
 */
export function randomFleet(terrainId, rng = Math.random) {
    ...
    for (const def of FLEET) {
      ...
      for (let anchor = 0; anchor < CELLS; anchor++) {
        for (const dir of ['h', 'v']) {
          const cells = canPlace(anchor, dir, def.len, terrainId, occupied);
          if (cells) options.push({ anchor, dir, cells });
        }
      }
  ...
  throw new Error(`could not place a fleet on terrain "${terrainId}"`);
}

/** Fill in whatever the player never placed — used when the placement timer expires. */
export function completeFleet(partial, terrainId, rng = Math.random) {
  const kept = [];
  const occupied = new Set();
  ...
  for (const spec of partial ?? []) {
    ...
    const cells = canPlace(spec.anchor, spec.dir, def.len, terrainId, occupied);
    if (!cells) continue;
    cells.forEach((c) => occupied.add(c));
    kept.push({ type: def.type, len: def.len, anchor: spec.anchor, dir: spec.dir, cells });
  }
  ...
  for (const def of FLEET) {
    ...
    if (!options.length) return randomFleet(terrainId, rng); // start over rather than ship a partial fleet
    const pick = options[Math.floor(rng() * options.length)];
    pick.cells.forEach((c) => occupied.add(c));
